...
export function Waveform({
  ...
  if (!audioData) {
    ...
    return <div style={{width, height}} className='bg-red-500' />;
  }
...
type WaveformCanvasProps = Pick<
  WaveformProps,
  'width' | 'height' | 'color' | 'lineWidth' | 'audioStartFrom'
> & {
  audioData: AudioData;
};
...
function WaveformCanvas({
  ...
  useEffect(() => {
    if (!canvasRef.current) {
      throw new TypeError(
        'No canvas attached to canvasRef. Please re-check the implementation.',
      );
    }
    ...
    if (!ctx) {
      throw new TypeError(
        'Unable to get CanvasRenderingContext2D for the canvas. Please re-check the implementation.',
      );
    }
    ...
    continueRender();
  }, [width, height, color, lineWidth, frequencyDataScaled, continueRender]);
...
const scaleAt = (
  ...
  const x =
    index >= copies ? (index + 1 - copies) / copies : (index + 1) / copies;
  return easing(x);
};
